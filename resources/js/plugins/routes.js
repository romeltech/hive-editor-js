...
function validateAccess(slug){
    let hasAccess = false;
    if(auth.userObject.status == 'active' && auth.userObject.role == 'superadmin'){
        hasAccess = true;
    }else if(auth.userObject.status == 'active' && adminOnly.includes(auth.userObject.role) == true && returnAccess(slug)){
        hasAccess = true;
    }
    return hasAccess;
}
