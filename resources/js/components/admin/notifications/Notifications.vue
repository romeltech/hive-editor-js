<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">Notifications</v-toolbar-title>
    </v-app-bar>
    <v-container class="py-8"  v-if="pageLoading == true">
      ...
      <v-row class="mt-2"> 
          ...
          <v-btn to="/d/notification/new" class="secondary mb-5">New Receiver</v-btn>
        </v-col>
...
export default {
  ...
  methods: {
    async getAllData(page) {
      let response = "";
      if(this.search){
          response = await axios.get("/d/notifications/fetch/"+this.showPerPage +"/"+this.search+ "/?page=" +page);
      }else{ 
          response = await axios.get("/d/notifications/fetch/"+this.showPerPage +"/-/?page=" +page);
      }
    ...
    searchData: async function () {
      ...
      if (this.search) {
        this.localStorage.setItem("vnotify", this.search); 
      }  
      ...
      if (
        ...
        await axios
          .get(
            "/d/notifications/fetch/" + this.showPerPage + "/" + this.search
          )
          .then((res) => {
     ...
     onPageChange: function () {
      this.$router
        .push(
          "/d/notifications/page/" + this.page
        )
        .catch((err) => {});
    },

    clearSearch: function (v) { 
       this.localStorage.setItem("vnotify", "");
      this.search = "";
  ...
  created() { 
     ...
     this.search = this.localStorage.getItem("vnotify");
    if (this.$route.params.page) {
