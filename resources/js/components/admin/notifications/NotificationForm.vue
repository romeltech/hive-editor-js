<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">{{ pagetitle }} Receiver</v-toolbar-title>
    </v-app-bar>
    ...
    objectdata: {
      handler(val, oldVal) {
        if (val != oldVal) {
          ...
          this.cardTitle = "Update Receiver";
  ...
  methods: {
    ...
    submit() {
      ...
      let dataForm = { data: this.usersObj };
        // Send data to save
        axios
          .post("/d/notification/save", dataForm)
          .then((response) => {
            this.sbOptions = {
              status: true,
              type: "success",
    ...
    confResponse(value) {
      if (value == true) {
        axios
          .post("/d/notification/delete/" + this.usersObj.id)
          .then((response) => {
            this.$emit("saved", true);
            this.sbOptions = {
              status: true,
