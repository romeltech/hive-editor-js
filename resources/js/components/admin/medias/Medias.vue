<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">News &amp; Article</v-toolbar-title>
    </v-app-bar>
...
export default {
  ...
  methods: {
    ...
    searchData: async function () {
      ...
      if (this.search) {
        this.localStorage.setItem("vmedia", this.search);
      }
    ...
    onPageChange: function () {
      this.$router.push("/d/posts/page/" + this.page).catch((err) => {});
    },

    clearSearch: function (v) {
      this.localStorage.setItem("vmedia", "");
      this.search = "";
