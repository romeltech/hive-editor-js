<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">Vehicles</v-toolbar-title>
    </v-app-bar>
    ...
    <v-container v-else class="mb-3 mx-auto" style="max-width: 1366px">
      ...
      <v-row class="mt-2">
        <v-col cols="12" class="py-0">
          <v-btn to="/d/car/new" class="secondary mb-5">New Vehicle</v-btn>
        </v-col>
              ...
              <template v-slot:default>
                <thead>
                  <tr>
                    ...
                    <th class="text-left">Cars</th>
                    <th class="text-left cursor-pointer" @click="OrderByField('year')">Year</th>
                    <th class="text-left">Color</th>
                    <th class="text-left">Chassis No.</th>
                    <th class="text-left cursor-pointer" @click="OrderByField('registration_expiry')">Reg. Expiration</th>
                    <th class="text-left cursor-pointer" @click="OrderByField('insurance_expiry')">Ins. Expiration</th>
                    <th class="text-left">KM Registered</th>
                ...
                <tbody v-if="Object.keys(items).length > 0">
                  <tr v-for="(item, index) in items" :key="index">
                    ...
                    <td>
                      <v-chip
                        v-if="
                          isExpired(item.registration_expiry) !== null &&
                          item.registration_expiry
                        "
                        small
                        :class="`${isExpired(item.registration_expiry)} `"
                      >
                        {{ formatDateHelper(item.registration_expiry) }}
                      </v-chip>
                      <div v-else>
                        {{
                          item.registration_expiry &&
                    ...
                    <td>
                      <v-chip
                        v-if="
                          isExpired(item.insurance_expiry) !== null &&
                          item.insurance_expiry
                        "
                        small
                        :class="`${isExpired(item.insurance_expiry)} `"
                      >
                        {{ formatDateHelper(item.insurance_expiry) }}
                      </v-chip>
                      <div v-else>
                        {{
                          item.insurance_expiry &&
...
export default {
  ...
  methods: {
    ...
    async getAllData(page,orderby=null) {
      ...
      if (this.search) {
        ...
        response = await axios.get(
          "/d/cars/fetch/" + this.showPerPage + "/-/"+sort+"/?page=" + page
        );
      }
    ...
    searchData: async function () {
      ...
      if (this.search) {
        this.localStorage.setItem("vcars", this.search);
      }
      ...
      if (this.search && this.search.length > 2) {
        ...
        await axios
          .get("/d/cars/fetch/" + this.showPerPage + "/" + this.search+"/"+sort)
          .then((res) => {
    ...
    onPageChange: function () {
      this.$router.push("/d/cars/page/" + this.page).catch((err) => {});
    },

    clearSearch: function (v) {
      this.localStorage.setItem("vcars", "");
      this.search = "";
  ...
  created() {
    this.search = this.localStorage.getItem("vcars");
    if (this.$route.params.page) {
