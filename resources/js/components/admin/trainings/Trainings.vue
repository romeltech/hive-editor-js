<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">Trainings</v-toolbar-title>
    </v-app-bar>
    ...
    <v-container v-else class="mb-3 mx-auto" style="max-width: 1366px">
      ...
      <v-row class="mt-2">
        <v-col cols="12" class="py-0">
          <v-btn to="/d/admin/training/new" class="secondary mb-5"
              ...
              <template v-slot:default>
                ...
                <tbody v-if="items && Object.keys(items).length > 0">
                  <tr v-for="(item, index) in items" :key="index">
                    <td>{{ item.id }}</td>
                    <td>{{ item.title }}</td>
                    <td>{{ item.categories ? item.categories.map((res) => {return res.title;}).toString() : ''}}</td> 
                    <td>
...
export default {
  ...
  methods: {
    ...
    searchData: async function () {
      ...
      if (this.search) {
        this.localStorage.setItem("vtraining", this.search);
      }
    ...
    onPageChange: function () {
      this.$router.push("/d/trainings/page/" + this.page).catch((err) => {});
    },

    clearSearch: function (v) {
      this.localStorage.setItem("vtraining", "");
      this.search = "";
  ...
  created() {
    this.search = this.localStorage.getItem("vtraining");
    if (this.$route.params.page) {
